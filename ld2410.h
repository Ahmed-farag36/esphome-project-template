...
class VersionSensor : public TextSensor
{ // , public Sensor {
public:
...
class TextInfoSensor : public TextSensor
{ // , public Sensor {
public:
...
public:
    ...
    ConfigSwitch *show_stats = new ConfigSwitch([this](bool b)
                                                { this->setShowStats(b); });
    ConfigSwitch *show_engineering_stats = new ConfigSwitch([this](bool b)
                                                            { this->setEngineeringMode(b); });
    ...
    void setup() override
    {
        ...
        set_timeout("setup-query", 1000, [this]()
                    { this->queryAll(); });
    }
    ...
    bool sendCommand(uint16_t cmd, char *commandValue = nullptr, int commandValueLen = 0)
    {
        ...
        std::vector<uint8_t> ld2410_conf = {0xFD, 0xFC, 0xFB, 0xFA, lowByte(len), highByte(len), lowByte(cmd), highByte(cmd)};
        if (commandValue != nullptr)
        {
    ...
    static std::string targetStateAsString(TargetState state)
    {
        ...
        case TargetState::NO_TARGET:
            result += "--";
            break;
        case TargetState::MOTION:
            result += "M-";
            break;
        case TargetState::STATIC:
            result += "-S";
            break;
        case TargetState::MOTION_AND_STATIC:
            result += "MS";
            break;
        default:
            char buf[5];
    ...
    static std::string commandWordAsString(uint16_t commandWord)
    {
        ...
        case CONFIG_END:
            result += "end Config";
            break;
        case SET_DISTANCE_AND_DURATION:
            result += "Set Range and Duration";
            break;
        case READ_PARAMETER:
            result += "Read Parameter";
            break;
        case ENGINEERING_START:
            result += "start engineering mode";
            break;
        case ENGINEERING_END:
            result += "end engineering mode";
            break;
        case SET_GATE_SENSITIVITY:
            result += "Rage Gate Config";
            break;
        case READ_FIRMWARE_VERSION:
            result += "Read Firmware Version";
            break;
        case SET_BAUD_RATE:
            result += "Set serial port baud rate";
            break;
        case FACTORY_RESET:
            result += "factory config reset";
            break;
        case RESTART:
            result += "Restart module";
            break;
        case SET_BLUETOOTH:
            result += "Bluetooth setting";
            break;
        case READ_MAC_ADDRESS:
            result += "read mac address";
            break;
        default:
            char buf[6];
    ...
    void handleConfData()
    {
        ...
        case READ_PARAMETER | RESPONSE:
        { // Read Parameter ACK
            ConfigurationParameterFrame *msg = (ConfigurationParameterFrame *)dataBuffer;
            publishIfChanged(maxMotionDistance, GATE_SIZE_CM * msg->maxMotionGate);
            publishIfChanged(maxStaticDistance, GATE_SIZE_CM * msg->maxStaticGate);
    ...
    void handle()
    {
        int av = available();
        if (av > 128)
        { // default buffer on ESP32 is 256 bytes
            ESP_LOGW(TAG, "More than expected bytes in buffer. %d - we are to slow! expecting %d",
                     av, expectedDataAmount);
        }
        while (expectedDataAmount && av >= expectedDataAmount && read_array(&(dataBuffer[dataBufferPos]), expectedDataAmount))
        {
